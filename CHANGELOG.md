...
- Project-level `docpilot.toml` defaults (library/index/manifest/trust/federation/output).
- Source provenance manifest (`.docpilot/source.json`) including canonical URL, requested/resolved refs, integrity, and trust signals.
- Optional source provenance in `cite` and `use` (`citation_details`).
- Security policy support via `docpilot.policy.json` with host, extension, and size/file limits.
...
- Surface extraction now caches parsed artifacts under `.docpilot/cache/surfaces`.
...
### Added
- Initial release of `docpilot` with `bootstrap`, `build`, `list`, `stats`, `search`, `open`, `cite`, and `use`.
