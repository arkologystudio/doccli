...
- Keep `doccli` core local-first, deterministic, and citation-backed.
...
- Add `doccli fetch <selector>` where selector can be a discovered candidate, package spec, repo URL, or docs URL.
...
- Add `doccli prep <query_or_url> [--version]`:
  - discover -> fetch -> build -> emit manifest.
- Keeps existing `doccli use` experience unchanged after prep.
...
1. `doccli discover <query> [--ecosystem <name>] [--max-results <n>] [--json]`
2. `doccli fetch <selector> [--version <v>] [--ref <sha|tag>] [--out <dir>] [--json]`
3. `doccli prep <query_or_url> [--version <v>] [--path <dir>] [--json]` (P1 but recommended)

## Data Contract Additions
1. Source manifest (`.doccli/source.json`)
...
- Add policy file support (`doccli.policy.json`) and safe defaults.
