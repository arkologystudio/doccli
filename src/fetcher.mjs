...
function resolvePolicy(policyPath = "") {
  ...
  const target = policyPath ? path.resolve(policyPath) : path.resolve("doccli.policy.json");
...
function assertHostAllowed(rawUrl, policy) {
  ...
  if (policy.blocked_hosts.some((entry) => String(entry).toLowerCase() === host)) {
    throw new CliError(
      ...
      "Update doccli.policy.json blocked_hosts"
    );
  }
  ...
  if (policy.allowed_hosts.length > 0 && !policy.allowed_hosts.some((entry) => String(entry).toLowerCase() === host)) {
    throw new CliError(
      ...
      "Update doccli.policy.json allowed_hosts"
    );
  }
}
...
function shouldSkipDirectory(name) {
  return [".git", "node_modules", ".doccli", ".next", "dist", "build"].includes(name);
}
    ...
    for (const entry of entries) {
      ...
      if (filesCopied > policy.max_files) {
        throw new CliError(
          ...
          "Adjust doccli.policy.json max_files"
        );
      }
      ...
      if (totalBytes > policy.max_total_bytes) {
        throw new CliError(
          ...
          "Adjust doccli.policy.json max_total_bytes"
        );
      }
...
function writeSourceManifest(snapshotDir, payload) {
  const sourceManifestPath = path.join(snapshotDir, ".doccli", "source.json");
...
export async function fetchLibrarySource({ selector, flags = {} }) {
  ...
  const cacheRoot = flags["cache-dir"] ? path.resolve(String(flags["cache-dir"])) : path.resolve(".doccli", "cache", "sources");
